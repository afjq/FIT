...
<!-- 头部导航栏模块 -->
	...
	<div style="width:2000px;border:1px solid black;background-color:#ddd;">
		<div style="color:red">
		</div>
     <table width="50%" border="1" cellpadding="3" cellspacing="1"
          ...
          <tr>
             ...
             <td>
                <a
		         href="${pageContext.request.contextPath }/coach/showAllCoachTrain?coachId=${coach.coachId}">返回课程页面
		        </a>
             </td>
          </tr>
          <c:forEach items="${requestScope.coachOrderList }" var="coachOrder" varStatus="statu">
          <tr>
             <td>${statu.count }</td>
             <td>${coachOrder.coachOrderCoach.coachName }</td>
             <td><fmt:formatDate value='${coachOrder.coachOrderCreatetime }' pattern='yyyy/MM/dd HH:mm:ss'/></td>
             
             <td><a href="${pageContext.request.contextPath }/cOrderDetail/getCOrderDetail?coachOrderId=${coachOrder.coachOrderId }">点我查看订单详情</a></td>
          </tr>
          </c:forEach>
       </table>
       </div>
